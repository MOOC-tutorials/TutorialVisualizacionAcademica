 ...
 * @@par:  	
 */
public class StudentVirtualFolder extends JFrame implements ActionListener{
	/**
     * Constants
     */
	...
	private JLabel cLabel = new JLabel("Consultar curriculo");
	
	private JButton curriculumB = new JButton(new ImageIcon("data/cur2.JPG"));
	
	private JLabel fLabel = new JLabel("Amonestaciones");
	...
	private ButtonGroup currics = new ButtonGroup();
	
	private JRadioButton student = new JRadioButton("Personal");
	
	private JRadioButton prog = new JRadioButton("Carrera");
	...
	private ProgramCurriculum pCurri;
	
	private StudentMail messages;
	
	private Inbox inbox;
	
	private StudentFault dicFault;
	
	public StudentVirtualFolder()
	{
		curriculum = new StudentCurriculum();
		pCurri = new ProgramCurriculum();
		messages = new StudentMail();
		dicFault = new StudentFault();
		inbox = new Inbox();
		currics.add(student);
		currics.add(prog);
    	...
    	label1.setBounds(15, 210, 280, 21);
    	curriculumB.setBounds(420, 260, 100, 100);
    	prog.setBounds(390, 360, 100, 21);
    	student.setBounds(505, 360, 100, 21);
    	cLabel.setBounds(550, 300, 240, 26);
    	message.setBounds(40, 400, 100, 100);
    	bInbox.setBounds(600, 400, 100, 100);
    	mLabel.setBounds(150, 440, 240, 26);
    	fault.setBounds(315, 400, 100, 100);
    	fLabel.setBounds(425, 440, 240, 26);
	...
	public void actionPerformed(ActionEvent event) 
	{
		...
		if( command.equals(CURRICULO))
		{
			...
			else
			{
				...
				JOptionPane.showMessageDialog(null, "Debe seleccionar una opci�n para deplegar el curriculo", "Error", 0);
			}
		}
...
//			dicFault.setVisible(true);
	...
	private void showProgramCurriculum() 
	{
		pCurri.loadCurriculumCourses();
		pCurri.setVisible(true);
		pCurri.setLastFrame(this);
		setVisible(false);
	}

	private void showStudentCurriculum() 
	{
		curriculum.loadCurricCoursesComboBox();
		curriculum.loadAreas();
		curriculum.loadPlannedCurriculum();
