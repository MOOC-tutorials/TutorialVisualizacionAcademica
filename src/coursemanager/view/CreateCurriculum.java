 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class CreateCurriculum extends JFrame implements ActionListener, Observer
{
	...
	public CreateCurriculum ()
	{
		...
		textArea1.setText("Sistema de manejo de Curriculos");
		textArea1.setFont(new Font("Serif", Font.BOLD, 25));		
...
//    	listeners
    	...
    	areas.setActionCommand(CBUA_COMBO);
	...
	public void actionPerformed(ActionEvent arg0)
	{
		...
		else if (comando.equalsIgnoreCase(CBUA_COMBO))
		{
	...
	private void moveoOut() 
	{
		...
		else
		{
			for (int i = 0; i <10 && !finished; i++)
			{
				...
				{			
					moved = manager.removeCourseCurriculum(lastSelected,i);
					foundSelection = true;
	...
	private void add ()
	{
		...
		int semester = sem - 1;
		CodRes added;
		added = manager.addCourseToCurriculum(addCourses[0].getText(),semester);
	...
	private void move(boolean moveUp)
	{
		...
		else
		{
			for (int i = 0; i <10 && !finished; i++)
			{
				if(courses[lastSelected][i].getBackground().equals(Color.YELLOW))
				{			
					moved = manager.moveCourseCurriculum(lastSelected,i,moveUp);
					if (moved == CodRes.SUCCESS)
					{
						JOptionPane pane = new JOptionPane();
		        		pane.setBorder(new javax.swing.border.LineBorder(new java.awt.Color(0, 0, 255), 3, true));
	...
	private void showError(CodRes added) 
	{
		if ( added == CodRes.ALREADY_EXISTS)
		{
			JOptionPane pane = new JOptionPane();
			pane.setBorder(new javax.swing.border.LineBorder(new java.awt.Color(0, 0, 255), 3, true));
			JOptionPane.showMessageDialog(null, "Curso ya agregado", "Error", 
		...
		else if ( added == CodRes.PREREQ)
		{
			JOptionPane pane = new JOptionPane();
			pane.setBorder(new javax.swing.border.LineBorder(new java.awt.Color(0, 0, 255), 3, true));
			JOptionPane.showMessageDialog(null, "Violaci�n de Prerrequisito(s)", "Error", 
					JOptionPane.ERROR_MESSAGE);
		}
		else if ( added == CodRes.COREQ)
		{
			JOptionPane pane = new JOptionPane();
			pane.setBorder(new javax.swing.border.LineBorder(new java.awt.Color(0, 0, 255), 3, true));
			JOptionPane.showMessageDialog(null, "Violaci�n de Correquisito(s)", "Error", 
					JOptionPane.ERROR_MESSAGE);
		}
		else if ( added == CodRes.INVALID_MOVE)
		{
			JOptionPane pane = new JOptionPane();
			pane.setBorder(new javax.swing.border.LineBorder(new java.awt.Color(0, 0, 255), 3, true));
			JOptionPane.showMessageDialog(null, "Movimiento inv�lido", "Error", 
					JOptionPane.ERROR_MESSAGE);
		}	
		else if ( added == CodRes.FULL)
		{
			JOptionPane pane = new JOptionPane();
			pane.setBorder(new javax.swing.border.LineBorder(new java.awt.Color(0, 0, 255), 3, true));
			JOptionPane.showMessageDialog(null, "Semestre lleno", "Error", 
					JOptionPane.ERROR_MESSAGE);
		}
